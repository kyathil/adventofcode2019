...
public class Day3Solution {
    ...
    public static void main(String[] args)
    {
        ...
        s.close();
        Scanner v = new Scanner(rightWire).useDelimiter(",");
        grid.remove(""+0+","+0);
        totaldist=0;
        x=0; y=0;
        while(v.hasNext())
        {
            ...
            if(temp.charAt(0)=='R')
            {
                ...
                {
                    Node n = new Node();
                    n.x=x+i; n.y=y; //might get doubles here, shouldn't matter
                    n.d=totaldist+i;
        ...
        int closest = Math.abs(matches.get(0).x) + Math.abs(matches.get(0).y);

        for(Node n:matches)
        {
            int i = Math.abs(n.x) + Math.abs(n.y);
            if(i<closest)
                closest=i;
        }
        System.out.println("Closest: " + closest);

        int shortest=matches.get(0).d + matches2.get(0).d;
        for(int i=0;i<matches.size();i++)
        {
            int d = matches.get(i).d + matches2.get(i).d;
            if(d<shortest)
                shortest=d;
        }
        System.out.println("Shortest found: " + shortest);
